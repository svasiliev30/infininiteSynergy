...
public class MyServerPostSignin extends RequestHandler implements HttpHandler {
    @Override
    public void handle(HttpExchange exchange) throws IOException {
        ...
        if ("POST".equals(exchange.getRequestMethod())) {
            ...
            boolean result = new ProductRepository().authenticationPerson(login, password, url);
            if (result) {
                String response = login + " you are logged in";
                goodAnswer(exchange, response);
            } else {
                String response = login + " you are not logged in";
                extractedException(exchange, response);
            }
        }
    }
